    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-xl text-gray-600 max-w-3xl mx-auto">
          Use our AI-powered classifier to identify your waste and get recycling
          recommendations. Earn eco points for every classification!
        </p>
      </div>
          ...
          <CardContent className="p-6 text-center">
            ...
            <div className="text-2xl font-bold text-secondary">
              {totalClassifications > 0
                ? Math.round(totalPoints / totalClassifications)
                : 0}
            </div>
      ...
      {/* Main Classifier */}
      <WasteClassifier onResult={setLatestClassification} />

      {/* Disposal request panel — pre-fills from latest classification */}
      <div className="mt-8">
        <DisposalPanel
          classification={latestClassification}
          onCreated={(created) => {
            // created may be { request: {...} } or the created doc depending on your API wrapper
            const id =
              (created &&
                created.request &&
                (created.request.requestId || created.request._id)) ||
              (created && (created.requestId || created._id)) ||
              null;
        ...
        {createdRequestId && (
          <div className="mt-4 p-3 bg-emerald-50 text-emerald-800 rounded-md">
            Request created: <strong>{createdRequestId}</strong>
          </div>
        )}
      </div>
            ...
            <div className="space-y-4">
                ...
                >
                  <img
                    ...
                    onError={(e) => {
                      ...
                      e.target.src = "/placeholder-image.png"; // Optional: add a placeholder
                    }}
                  />
              ...
              {classifications.length > 10 && (
                <p className="text-center text-sm text-gray-500 pt-4">
                  Showing recent 10 classifications out of{" "}
